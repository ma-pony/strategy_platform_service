...
class TestStrategyPairMetricsAdminColumnConfig:
    ...
    def test_column_list_contains_required_fields(self) -> None:
        ...
        required_fields = {
            ...
            "sharpe_ratio", "trade_count", "data_source", "last_updated_at",
        }
        # 检查必需字段是否均存在
        for field in required_fields:
            assert any(
                (hasattr(c, "key") and c.key == field) or str(c) == field
                for c in col_list
    ...
    def test_column_searchable_includes_pair(self) -> None:
        ...
        assert any(
            (hasattr(c, "key") and c.key == "pair") or str(c) == "pair"
            for c in searchable
        )
    ...
    def test_column_searchable_includes_timeframe(self) -> None:
        ...
        searchable = StrategyPairMetricsAdmin.column_searchable_list
        assert any(
            (hasattr(c, "key") and c.key == "timeframe") or str(c) == "timeframe"
            for c in searchable
        )
    ...
    def test_column_sortable_includes_last_updated_at(self) -> None:
        ...
        assert any(
            (hasattr(c, "key") and c.key == "last_updated_at") or str(c) == "last_updated_at"
            for c in sortable
        )
    ...
    def test_column_sortable_includes_total_return(self) -> None:
        ...
        sortable = StrategyPairMetricsAdmin.column_sortable_list
        assert any(
            (hasattr(c, "key") and c.key == "total_return") or str(c) == "total_return"
            for c in sortable
        )
